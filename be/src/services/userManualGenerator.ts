...
import { encode } from 'gpt-tokenizer'; // Assuming you have gpt-tokenizer
...
// --- Helper for Extracting User-Relevant Information (from previous correct version) ---
...
Be concise and provide bullet points or short descriptions.`;
    const userPrompt = `File Content:\n\n\`\`\`\n${content}\n\`\`\``;
    try {
        const res = await axios.post('https://api.openai.com/v1/chat/completions', {
            model: 'gpt-4o-mini', messages: [{ role: 'system', content: systemPrompt },{ role: 'user', content: userPrompt }],
...
// --- Helper for Assembling the User Manual (from previous correct version) ---
...
If information is sparse or developer-focused, create the best user manual possible, noting missing info if critical. Do NOT invent features.`;
    ...
    try {
        const res = await axios.post('https://api.openai.com/v1/chat/completions', {
            model: 'gpt-4o-mini', messages: [{ role: 'system', content: systemPrompt },{ role: 'user', content: userPrompt }],
            max_tokens: 3800, temperature: 0.3,
        }, { headers: { Authorization: `Bearer ${OPENAI_API_KEY}` } });
...
export async function generateUserManual(
    ...
    try {
        ...
        const POTENTIALLY_USEFUL_EXTENSIONS = /\.(md|txt|html|rst|adoc|asciidoc|tex|rtf|ini|toml|yaml|conf|cfg|sh|ps1|js|ts|jsx|tsx|py|java|cs|go|rb|php|swift|kt|c|cpp|h|hpp|xml|json|sql|graphql|feature)$/i;
        ...
        const relevantFiles = fileContents.filter(f => {
            ...
            if (POTENTIALLY_USEFUL_EXTENSIONS.test(f.path) || f.path.toLowerCase().includes('readme') || f.path.toLowerCase().includes('license') || f.path.toLowerCase().includes('contributing')) {
                return true;
            }
            return false;
        });
        ...
        if (extractedInfo.every(info => info.startsWith("Error:") || info.includes("Primarily for developers") || info.includes("unsuitable for user manual summary"))) {
            ...
            const placeholderContent = `# User Manual for ${repoFullName}\n\n${noMeaningfulInfoMsg}`;
            return { success: false, message: noMeaningfulInfoMsg, markdownContent: placeholderContent, error: noMeaningfulInfoMsg };
        }
