...
/* --- recent --- */
documentsRouter.get('/recent', requireLocalJWT, async (req: AuthRequest, res: Response) => {
    try {
        const { rows } = await pool.query(
...
documentsRouter.put('/:id', requireLocalJWT, async (req: AuthRequest, res: Response) => {
    ...
    try {
       ...
       WHERE id = $6 AND owner_id = $7`,
        ...
        if (!rowCount) return res.status(403).json({ message: 'Not your document' });
...
documentsRouter.post('/analyze-repository', async (req: Request, res: Response) => {
    ...
    try {
        const result = await analyzeRepository(repoFullName, token, selectedBranch);
